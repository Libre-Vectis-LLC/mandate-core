# Mandate Core TODO (API-first, audit-friendly)

## Completed
- [x] Hashing helpers with `Hash256`/`Hash512`; SHA3-256 for content hashes and BLAKE3-XOF-512 for `nazgul` ring consensus hashes.
- [x] Contextual BLSAG wrapper: `SignatureKind` (anonymous/authoritative), compact/archival, `MasterKeypair` ring-context derivation (BLAKE3-XOF-512 ring hash + SHA3-based KDF paths); `KeyImage` kept uncompressed.
...
- [x] Hash/serialization policy: canonical JSON (sorted keys, no whitespace) + domain-separation prefixes. Keep SHA3-256 for 256-bit content hashes and BLAKE3-XOF-512 where the protocol needs extended digest material; digest roles stay explicit.
...
- [x] Docs: expand `lib.rs`/README to spell out audit chain rules, ring reconstruction contract, current hash policy, storage concurrency model (single-writer append, no optimistic tokens), and WASM notes.

## Next (design alignment with latest dev drafts)
- [x] KDF/Derivation unification: add HKDF-SHA3 helpers (default 256, optional 512). Provide domain/context builders for org/event/poll keys.
...
- [x] Storage wiring: map EventStore/RingView/BanIndex traits to service handlers (with in-memory mock for tests).
